...
## Коннекторы. Настройка и получение ключей
...
## Ресурсы и сервисы доступа.
Система StoryCLM позволяет использовать следующие ресурсы:

# [Контент](#Сервис-доступа-к-контенту)
# [Пользователи](https://github.com/storyclm/Java-SDK#Сервис-доступа-к-данным-пользователя)
# [Таблицы](https://github.com/storyclm/Java-SDK#Сервисы-доступа-к-таблицам)

Для доступа к каждому ресурсу коннектор предоставляет соответсвующий сервис:
...
Асинхронный метод-фабрика сервиса строготипизированного доступа к табличным данным StoryCLM. 
...
Асинхронный вызов для получения сервиса строготипизированного доступа к табличным данным StoryCLM.

**Пример** (используется clientConnector, полученный в предыдущем примере):

    StoryCLMTableService<StoryLog> slogService = clientConnector.GetTableService(StoryLog.class, "TableName").GetResult();
